...
**With reactive forms.** `ArenaSwitchControl` from `@dravensoft/arena-angular/forms` binds the switch to a boolean form control. With `confirm` set a press reports nothing to the form: answer `requestChange`, then call `setValue` with the state you confirmed. Binding the member the form replaces as well is a caller bug, and the layer warns about it once.
