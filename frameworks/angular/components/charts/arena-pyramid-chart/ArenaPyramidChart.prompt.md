...
```ts
readonly byAge = computed<ArenaSeries[]>(() => [
  { label: 'Women', values: this.women() },
  { label: 'Men', values: this.men() },
]);
```
...
```html
<arena-pyramid-chart label="Population by age band" [labels]="bands()" [series]="byAge()" />
```
