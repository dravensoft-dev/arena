...
# Arena charts components, the Angular layer
...
| `ArenaChartCard` | A titled card frame around a chart, with an optional actions slot in its head. | `title` `headingLevel` `actions` `content` | [`ArenaChartCard.prompt.md`](./arena-chart-card/ArenaChartCard.prompt.md) |
