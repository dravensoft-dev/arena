...
| `ArenaTableCell` | One cell of an ArenaTableRow. It draws the cell box (the padding, the alignment and the mono/gold treatment its column asks for, and in card mode the label/value pair or the full-width block), and shows whatever the consumer put in it. Its column config, its layout and its place in the grid's keyboard order come from ArenaTable and ArenaTableRow and are members of no contract. | `content` `href` `navigate` | [`ArenaTableCell.prompt.md`](./arena-table-cell/ArenaTableCell.prompt.md) |
