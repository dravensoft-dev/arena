...
- **Pass `interactive` alongside `(click)`, or the row is inert.** The flag is what makes the card
  shape a `role="button"` tab stop with an Enter/Space handler. The member exists because this layer cannot ask whether an output has subscribers. `OutputEmitterRef.listeners` is private, so the shape has to be declared rather than detected. Before it existed, a clickable card row here was
...
you write IS the row. An `<arena-table-row>` wrapping a `<tr>` does not work, and the reason is worth having before you reach for it. `display: contents` fixes the **box** tree rather than the **DOM** tree, so a `<tr>` that is not a DOM child of `<tbody>` never forms the native mapping at all. Under server rendering it is worse, because the markup is serialized and re-parsed and the HTML parser foster-parents a non-table element straight out of the table.
...
Where the row sits, which columns its cells are set against, and where the grid's cursor is all live on `ArenaTableState`. The table provides that state, and this component injects it. None of it is
