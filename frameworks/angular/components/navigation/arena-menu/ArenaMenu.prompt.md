...
The clipping is the whole reason for the overlay. A menu's canonical home is an overflow row in a table or a card, and an absolutely-positioned panel is clipped by the first `overflow: hidden` ancestor. Arena uses the CDK for **position only**: the roles, the keys and the focus are
...
on it would reach nobody. The menu also stops its own native click, so the activation is bound to the resolved control directly and never to the host.
...
- **Do** put the marker on the element `arena-menu` itself receives. Wrapping the control in
  ...
  inside it. The focusable control is still found by search, so the attributes land in the right place either way. What a misplaced marker loses is the projection, and the menu then renders no trigger at all.
...
- **The trigger must be a control, not a picture of one.** An `arena-avatar`, a `<span>` or anything else that takes no focus receives the listeners and answers no key. The menu opens on a pointer, and a reader on a keyboard never reaches it. Arena reports that once at
...
- **Don't** use `shortcut` expecting Arena to bind the key. The member is display only, and the contract says so. The host binds it or nothing does.
...
- One Enter opens it **once**, and one Space does too. The double-open is exactly what the `preventDefault` is there to stop, and only a browser synthesizes the click that would cause it.
