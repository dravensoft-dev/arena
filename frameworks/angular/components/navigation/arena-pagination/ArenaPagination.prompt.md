...
Jumps between pages of a large set, the companion to a table or a long list. The control renders a *window* over the pages rather than all of them, and elides the rest with a single `…`.
...
`change` carries the new 1-based page. The output never fires for the page already shown, nor for a page outside `1..pageCount`. A handler can set its signal without re-checking either.

How wide the window is comes from the design layer rather than from this component. `--limit-pagination-siblings` says how many pages flank the current one, and the total width is derived from it at the point of use. Both layers read the same token.
...
- Don't hide the arrows at the ends. The two are disabled there, which keeps the strip's width from changing as a consumer pages through it.

**By hand, in real Chromium**, at `frameworks/angular/components/navigation/arena-pagination/ArenaPagination.demo.generated.html`, served with `bun run demos`: - Walk the 20-page strip to the middle and watch the window elide on **both** sides. Then walk to each end and watch it elide on one side only.
