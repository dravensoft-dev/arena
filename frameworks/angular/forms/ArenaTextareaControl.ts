...
@Directive({
  selector: 'arena-textarea[formControl], arena-textarea[formControlName], arena-textarea[ngModel]',
  standalone: true,
  providers: [{ provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => ArenaTextareaControl), multi: true }],
  host: { '(focusout)': 'touched()' },
})
export class ArenaTextareaControl implements ControlValueAccessor {
