...
| `--strict` | Exit 1 on a report rather than writing anyway. Bare, it holds every kind; `--strict=contrast,audit` holds the kinds you name, out of `components`, `contrast`, `ramp`, `glyph`, `markers`, `audit` and `environment`. **Name them when one of them is a decision you already made**: a brand under 4.5:1 is measured and deliberate, and one switch over all of them would make it the price of holding the rest in CI. `environment` is the kind that says the run is outside an Arena package, so the icons Arena draws itself went uncounted, which is a fact about where the command ran rather than about any glyph. |
