...
| `--strict` | Exit 1 on a report rather than writing anyway. Bare, it holds every kind; `--strict=contrast,audit` holds the kinds you name, out of `components`, `contrast`, `ramp`, `glyph`, `markers` and `audit`. **Name them when one of the six is a decision you already made**: a brand under 4.5:1 is measured and deliberate, and one switch over all six would make it the price of holding the other five in CI. |
