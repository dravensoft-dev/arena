One destination in an `ArenaBottomNav`: glyph above label, an equal share of the bar's width. Which one is
...
- **Don't** write it outside an `ArenaBottomNav`. Nothing injects the active id there, so no destination is
  ever marked current.
