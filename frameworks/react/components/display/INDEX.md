...
| `ArenaCard` | Surface container. Hairline border on the base surface scale; depth comes from the shadow, never a gradient. | `children` `interactive` `disabled` `href` `action` `title` `headingLevel` `eyebrow` `floating` `accent` `onClick` | [`ArenaCard.prompt.md`](./arena-card/ArenaCard.prompt.md) |
