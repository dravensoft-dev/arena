...
<ArenaScroller label="Recently landed lots" itemWidth="calc(var(--sp-1) * 62)">
  {arrivals.map((lot) => <LotCard key={lot.id} lot={lot} />)}
</ArenaScroller>
```
...
row of the same cards agree about how wide a card is and a voice re-answers both at once.
...
required and guarded too: an empty row is a tab stop over nothing.
