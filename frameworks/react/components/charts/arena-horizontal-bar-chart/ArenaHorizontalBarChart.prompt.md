...
<ArenaHorizontalBarChart label="Open issues by area"
  labels={['Documentation', 'Onboarding', 'Billing', 'Search']}
  series={[{ label: 'Open issues', values: [42, 31, 18, 12] }]} />

{/* stacked, when the segments are parts of one total */}
<ArenaHorizontalBarChart label="Spend by quarter" labels={quarters} stack
  series={[{ label: 'Cloud', values: cloud }, { label: 'Licences', values: licences }]}
...
Transposing the axes changes what four members mean, which is why `ArenaBarChart` has no
