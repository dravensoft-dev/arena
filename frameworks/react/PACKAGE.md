...
restating what a shipped slot already paints, a projection marker written without the directive
that binds it, and a glyph Phosphor does not draw. `contrast` and `ramp` are left out on purpose,
