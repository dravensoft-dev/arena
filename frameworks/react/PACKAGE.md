...
**What comes down with it.** First `react` and `react-dom`, which you already have. Then `@phosphor-icons/web`, which you may not. Arena's icons are Phosphor class names a component renders, never SVGs it bundles. The font is installed alongside the package rather than bundled inside it. There is no runtime dependency and no CSS toolchain to run. Every component's CSS ships compiled, written against Arena's own class names and Arena's own tokens and against nothing else. A project running its own utility framework cannot collide with it in either direction.
...
**A `<head>` is not this package's to write, and that is the answer.** `@dravensoft/arena-react` renders no title, description, canonical or `og:*` pair. Those stay wherever your application already owns them, which is your router or your metadata framework rather than a component library. Nothing here competes with that or has to be turned off for it.
...
The line above is the whole install. `react`, `react-dom` and `@phosphor-icons/web` are peer dependencies,
...
The command it ships is plain JavaScript reading `node:fs`, `node:path` and `node:url`. The floor is the support window rather than a capability, and a project on a supported Node installs without an engine warning.
...
The prefix is the name rather than a decoration on it, which is what lets `"components": "auto"` tell a component of Arena's from one of yours. A `<Card>` in your own source belongs to whoever wrote it, and an `<ArenaCard>` belongs here.
...
**Put your spacing and sizing on an element you wrote, and let the Arena element be its child.** Arena draws no outer margin on anything. The air between two components stays yours to place. No component takes a `className` or a `style` of yours, so there is no route into an Arena element's own box, by design.

React renders the real element in almost every case, so a `.row > * { ... }` rule of yours does
reach it. `ArenaTabs` is the exception that makes the rule worth stating. That component returns a fragment and puts no element of its own in the DOM at all. A rule written against it matches the tablist and the panels separately, or nothing. Wrap it, the way you would wrap anything else
whose layout is yours.

**Write the rule the same way wherever you write it.** A component's own element is Arena's, and what it renders is free to change. Today it is the real element. A component that wraps, or that renders none, owes you no warning, because that element was never something to lay out.
...
that CSS wrapped in the reserved cascade layer. The sheet leads with the layer order itself. A bundler that emits one `<link>` per stylesheet and does not keep your import order cannot change what wins.
...
**A TypeScript project declares what a stylesheet is before it can import one.** Those three lines are a bundler's idiom rather than TypeScript's. A `.css` specifier resolves to no module, so under `strict` the compiler reports TS2307 on every one of them. The build stops at the file that wires Arena in. Most toolchains already ship the declaration and the fix is to reference it,
...
The second line is the same problem one asset later. `ArenaAppLogo` takes the mark as a node you pass, and a bundled `import mark from './mark.svg'` is a specifier TypeScript resolves no better than a stylesheet. Passing the path as a bare string in `src` avoids the question, and gives up the hashed filename a build would have written. Neither declaration is Arena's to ship: an
...
The command counts a component as drawn when you import it from this package or open its tag. The count adds what Arena draws on your behalf, because an `ArenaTable` renders an `ArenaPagination` and an `ArenaSelect` you never wrote. The command prints both counts, and names on stderr anything it saw and could not place.
...
is generated wholesale rather than curated, and **carries no compatibility promise**. The symbol carries the prefix too, since the convention is about the name rather than about the promise. Reading `Arena` on a symbol tells you where it comes from, and never that it is yours to depend on. What this
