...
- **An accent is never ink on a wash of that same accent.** One colour as both ink and ground caps the ratio at what the ink reads against the surface itself, so no percentage repairs it and any palette whose accent sits mid-lightness fails structurally. **The cut is which half carries the meaning.** Where the accent is the STATE, a pressed `ArenaIconButton`, an active navigation row, a badge's tone, the wash carries it and the ink is the body ink; where the accent is the MEANING, danger read as text, the ink carries it and a hover moves the neutral fill rather than washing the token in its own colour. `check:text-contrast` holds it over every component sheet in both themes.
