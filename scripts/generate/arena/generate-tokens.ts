...
export const PLUGIN_DIR = 'plugin-style-store';
...
export const SOURCES = [...new Set(FILES.flatMap((file) =>
  file.blocks.map((b) => `${(b as { dir?: string }).dir ?? DESIGN_DIR}/${b.source}`)))];
...
export const node = {
  ...
  reads: [...new Set([
    ...SOURCES,
    ...Object.values(RESOLVES_AGAINST).flat().map((source) => `${DESIGN_DIR}/${source}`),
  ])],
...
function scriptSources() {
  return [...new Set(FILES.flatMap((f) => f.blocks.map((b) =>
    [b.source, (b as { dir?: string }).dir ?? DESIGN_DIR] as const)))];
}

export async function collectScriptTokens() {
  const out = [];
  for (const [source, dir] of scriptSources()) {
...
export function designPath(source: string, dir = DESIGN_DIR, on: PathModule = nodePath) {
  return toPosix(on.join(root, ...dir.split('/'), source), on);
}
...
async function load(source: string, dir = DESIGN_DIR) {
  const against = (RESOLVES_AGAINST as Record<string, string[]>)[source] ?? [];
  const sd = new StyleDictionary({
    source: [...against.map((s) => designPath(s)), designPath(source, dir)],
