...
| `check-surface-parity.ts` | two slots that draw the same surface stop agreeing about it. `ArenaUnauthCard`'s `panel` types out by hand the background, border width, border colour, radius and overflow that `ArenaCard`'s `root` draws, and nothing else compares one manifest to another: a change to one moves that one alone and leaves the other on whatever it had, silently, until somebody notices by eye. Sharing a recipe was refused on its own terms, since `ArenaUnauthCard`'s padding split is not `ArenaCard`'s, so what is checked instead is that the surface halves agree. `PAIRS` names each pair with the reason it is one, and `SURFACE` is the property prefixes a pair has to match on. |
