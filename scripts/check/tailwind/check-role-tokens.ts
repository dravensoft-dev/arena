...
export const SCALE_UTILITIES = new Map<string, string>([
  ...
  ['uppercase', 'case-eyebrow or case-label, so an extension can set the register in sentence case'],
...
export const SCALE_USES = new Map<string, string>([
  ...
  ['ArenaButton:root:bg-base-200', 'the fill of a secondary button, and the hover of a ghost one: a control the user presses rather than a region something is placed inside. An extension that flattens its surfaces onto the page is saying its regions are carried by air, and a button that went with them would stop looking pressable'],
  ['ArenaIconButton:root:bg-base-200', 'the same, as the hover fill of a control with no resting one'],
  ['ArenaBulkActionBar:action:bg-base-200', 'the same, as the hover fill of a control INSIDE a floating surface, which is why it must not follow that surface when an extension moves it'],
  ['ArenaInput:field:bg-base-200', 'the readonly state of a field, which says a control cannot be typed into by matching the surface it sits on. It is a state of a control, not the surface itself, and an extension that moved it would be re-answering readonly rather than re-answering grouping'],
  ['ArenaTextarea:field:bg-base-200', 'the same readonly state, for the same reason'],
  ['ArenaSheet:head:px-5', 'a sheet is pinned to the viewport edge, so its padding is a fit constraint rather than a statement about air. An extension with a lever here could push its content off the screen, which is why the floating tier has no padding role'],
  ['ArenaSheet:body:px-5', 'the same sheet, the same constraint'],
  ['ArenaSheet:foot:px-5', 'the same sheet, the same constraint'],
  ['ArenaAvatar:box:font-extrabold', 'a monogram rather than a heading: two letters standing in for a face, weighted to read at 12px inside a circle. An extension that lightened its headings has said nothing about initials'],
  ['ArenaAppLogo:name:tracking-tight', 'the wordmark, which is the brand set as artwork rather than a heading in the document outline. It tracks with the mark beside it and follows no extension'],
  ['ArenaTextarea:field:leading-body', 'the leading of text a user TYPES, which follows the control the caret sits in rather than the prose a page sets. Moving it with a reading extension would reflow a form field under somebody mid-sentence'],
  ...
  ['ArenaAppLogo:name:font-display', 'the wordmark, which is the brand set as artwork rather than a heading in the document outline. It follows the mark beside it and no extension'],
  ['ArenaAppLogo:name:uppercase', 'the same wordmark: its capitals are how the brand is drawn, and an extension that sets its eyebrows in sentence case has not renamed the company'],
  ...
  ['ArenaStatCard:value:text-h2', 'the figure a stat card exists to show, which is data set large and not a title of anything. An extension re-pitching the titles on a page has said nothing about how big a number is, and binding this would have moved every dashboard the first time one did'],
