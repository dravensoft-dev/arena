/* What travels inside the MCP tarball so an agent in somebody else's editor can read the language
 * offline, and how a path in it is rewritten. It imports only node:path, for the reason
 * package-exclusions.ts imports nothing: the publish guard asks it what a package carries in a job
...
export function payloadDir(layer: string) {
  return `${AGENT_DIR}/${layer}`;
}
