...
export const AXES: Axis[] = [
  {
    ...
    rows: [
      ...
      { answer: 'pnpm', evidence: 'verified',
        note: 'the strict layout is the one worth naming, because it is the one that would break a package assuming otherwise. Installed under pnpm, the command still resolves the icon font through the symlinked store' },
      { answer: 'bun', evidence: 'verified',
        note: 'the install npm takes, unchanged. Bun is also what Arena is built with, which is a fact about Arena rather than a requirement on you' },
  ...
  },
  {
    ...
    rows: [
      { answer: 'an import from a module of yours', evidence: 'verified',
        note: 'the idiom this page shows. The import is an instruction to a bundler rather than to TypeScript, which is why a project may need a declaration before it typechecks' },
      ...
      { answer: 'the global styles list a framework CLI takes', evidence: 'allowed',
        note: 'the barrel named as a global style rather than imported from a module, which is how a CLI-driven workspace usually takes one' },
