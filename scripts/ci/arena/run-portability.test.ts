...
import { SUPPORTED_OS, SUPPORTED_OS_NAMES } from './supported-os.ts';
...
test('the workflow matrix is exactly the declared list, in both directions', () => {
  ...
  assert.ok(matched, 'portability.yml declares no os matrix');
  const legs = (matched[1] ?? '').split(',').map((one) => one.trim()).filter(Boolean);

  assert.deepEqual(legs.slice().sort(), SUPPORTED_OS_NAMES.slice().sort(),
...
test('every supported OS carries a reason, and ubuntu says why it is there twice over', () => {
  for (const [name, why] of Object.entries(SUPPORTED_OS)) {
    assert.ok(why.length > 40, `${name} has a label rather than a reason`);
  }
  assert.match(SUPPORTED_OS['ubuntu-latest'] ?? '', /single spelling/);
});
