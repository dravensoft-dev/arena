...
The register of one page a stranger fills in once. The page carries who they are meeting, a month to pick a day from, a column of times, and a short form that ends in a single irreversible button. The entry is measured on a commercial scheduling product, and what is written down here is what the measurement found.

Take this entry when the product is a form somebody outside the organisation completes and will see once. The whole screen is then a question and its answer.
...
| **How hard a field is drawn** | `bw-field`, `bw-control`, `edge-field`, `edge-control` | **the strong border width on both**, the muted text colour on a field and the accent on a control | the pair is the entry's whole identity. A field somebody has never seen before has to look like a place to type before it is focused, and a hairline on a white card does not |
...
**The drawn field edge and the resting shadow are the pair that carries this register.** Both say the same thing in two places. Here is a thing to fill in, and it is standing on something else.
...
**Two decisions are painted by hand**, in `plugin.css` beside this page. One is a field label read at the size and strength of an answer, rather than held back to a caption. The second is the hint under it, kept quieter without being smaller than the answer. A level is not a colour, so no colour role could have
carried either. That sheet says why.

## What it does not bring

The palette and the fonts are the consumer's. This register spends its accent in three places at once: the selected day, the control edge and the one button. A replacement accent changes it more than a replacement face does.

The copy, the brand mark, the availability rules and the question of being found from outside are still open. This register almost always answers the last of those yes, because a booking page is reached from outside or it is reached by nobody.
