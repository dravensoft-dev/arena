...
The register of a product that sells what somebody else made. The product carries a grid of listings that are mostly photograph, and a price read at a glance. Beside those sit a marker saying why this one is different, and a buy column ending in one button. The entry is measured on a commercial handmade marketplace, and what is written down here is what the measurement found.
...
**The portrait crop and the drawn control edge are the pair that carries this register.** Everything else is a shop being ordinary. Those two are what stop the same components reading as a console.
...
The app bar, the site footer, the figure, the grid, the scroller and the section. Then the breadcrumbs, the pagination, the segmented control, the tag, the select, the input and the button. Then the key value list, the alert, the avatar and the sheet. `frameworks/react/INDEX.md` names each under its category.
...
- **The control laid over a picture**, the save on each card. Arena's icon button fills with the accent in its solid variant. A plugin selects by part and never by the value of a variant, so that difference cannot be painted. The shop draws the control and stands it on `overlay-media`,
  which is what
...
**Three decisions are painted by hand**, in `plugin.css` beside this page. The three are where a marker sits on a frame, how much air the bar spends on itself, and a trail pitched as chrome. That sheet says which
and why, and it is the copy that stops being true the day a rule is dropped.

## What it does not bring

The palette and the fonts are the consumer's. This register leans on the palette harder than the others, because the accent is the price and the marker at once. A replacement accent changes more here than a replacement face.
